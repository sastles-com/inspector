...
def main():
    parser = argparse.ArgumentParser(description="KIZKI Scratch Detector for general images")
    parser.add_argument("input", help="Path to the input image file")
    parser.add_argument("--output", "-o", help="Path to save the result image", default=None)
    parser.add_argument("--block-size", "-b", type=int, default=16, help="Block size for KIZKI algorithm (default: 16)")
    ...
    # KIZKI 検出器の初期化と実行
    detector = KizkiDetector(block_size=args.block_size, pre_blur_sigma=args.sigma)
    ...
    if args.output:
        ...
        # デフォルトの出力名: {original_name}_kizki.png
        base, _ = os.path.splitext(args.input)
        output_path = f"{base}_kizki.png"
    ...
    if args.show:
        ...
        cv2.imshow("Original (Gray) vs KIZKI Pop-out", combined)
